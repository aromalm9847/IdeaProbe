  ...
  return (
    <section className="py-16 max-w-2xl mx-auto px-4">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-white mb-2">This Week's Top Scoring Ideas</h2>
      ...
      <div className="bg-surface border border-border rounded-2xl overflow-hidden">
        {loading ? (
          <div className="space-y-0">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex items-center gap-4 px-6 py-4 border-b border-border last:border-b-0 animate-pulse">
                ...
                <div className="w-10 h-6 bg-surface-2 rounded-full" />
              </div>
            ))}
          </div>
        ) : (
          <div>
            {entries.map((entry, i) => (
              <div key={i} className="flex items-center gap-4 px-6 py-4 border-b border-border last:border-b-0 hover:bg-surface-2 transition-colors">
