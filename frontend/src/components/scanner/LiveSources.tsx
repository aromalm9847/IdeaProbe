    ...
    <div>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
        {sources.map((source, i) => (
          <div key={i} className="bg-white rounded-xl p-3 flex flex-col gap-1 border border-slate-100">
            <div className="flex items-center gap-2">
              <span className="text-lg">{PLATFORM_ICONS[source.platform] || '🌐'}</span>
              <span className="text-slate-800 text-sm font-medium">{source.platform}</span>
            </div>
            <div className="text-slate-500 text-xs">{source.count} signals found</div>
          </div>
        ))}
      </div>
      {sampleSignal && (
        <div className="bg-white border border-slate-100 rounded-xl p-4">
          ...
          <p className="text-slate-600 text-sm italic">"{sampleSignal.sample_signal}"</p>
        </div>
      )}
    </div>
  )
